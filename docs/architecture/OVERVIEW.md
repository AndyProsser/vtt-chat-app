...
- **`ddb/`** — DDB auth and extraction: cobalt cookie → JWT exchange, Character Service calls, DOM extraction helpers and the TypeScript types for what comes back. Consumed by `overlay-ui` (extraction) and `backend` (JWT verification, character caching).
