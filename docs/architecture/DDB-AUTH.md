...
| Firefox (Linux) | appeared to work on an isolated devtools probe |

The Firefox UA was briefly applied app-wide (`consts::LINUX_UA_OVERRIDE` in `tauri-client/src-tauri/`), but a full real login attempt with real credentials still failed the same way — so it was **reverted along with the Chrome attempt**; the app is back to WebKitGTK's real, unmodified UA. Treat the earlier "Firefox works" result as unconfirmed, not disproven: it may have looked like it worked only because it was an isolated, low-volume probe early in a testing session that (see below) plausibly degraded the source IP's standing over time regardless of UA.

**Root cause, revised.** A full HAR capture of the failing request (`POST /api/login`) shows a generic `403` with `Server: NotYours` and no specific error detail — the shape of a WAF/bot-manager decision, not an application-level validation error. The CSRF token in the POST body legitimately differs from the `_csrf` cookie value, but that's normal for synchronizer-token CSRF schemes (the cookie is a session-bound secret, not something string-compared to the body token) — **not** the bug it initially looked like. Akamai's `_abck` cookie is reissued even on this blocked response, meaning it's still actively risk-scoring. The most likely explanation at this point: **the login endpoint was hit an unusual number of times from the same source IP within about two hours** during this investigation (real UA, multiple spoofed UAs, several full app launches, direct `curl` probes) — Akamai-class bot managers commonly escalate risk scores with request *volume* from a source, independent of which UA is presented. That would explain the inconsistent per-UA results better than a stable UA-based rule: an early, isolated Firefox-UA probe looked clean; a later real submission — after far more requests had landed from the same IP — failed regardless.

**Current status: paused for a cool-down, not resolved.** No UA override is active; the app uses WebKitGTK's real UA. Before drawing further conclusions, retest later (risk/rate scores of this kind typically decay over minutes to hours) with minimal attempts against the real login flow, ideally without repeated probing beforehand. **Steam/Google/Apple OAuth remains the standing recommended path regardless of outcome** — those flows never touch this endpoint at all, need no UA games, and cover most of this app's actual audience (D&D players skew heavily toward already having a Steam account).
