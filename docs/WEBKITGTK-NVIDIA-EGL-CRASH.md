...
- NVIDIA, signature 2: [forum report](https://forums.developer.nvidia.com/t/egldestroycontext-segfault-in-libnvidia-eglcore-580-branch-during-ordinary-gl-context-teardown-likely-same-class-as-bug-5701801/382969), referencing their existing bug 5701801 — still an independently real driver bug, just no longer reachable via this trigger once 321683 lands.
...
Draft issue writeups for both signatures are at [`docs/issue-drafts/webkitgtk-null-pointer-backing-store.md`](issue-drafts/webkitgtk-null-pointer-backing-store.md) and [`docs/issue-drafts/nvidia-egldestroycontext-segfault.md`](issue-drafts/nvidia-egldestroycontext-segfault.md) — both filed; see [Filed reports](#filed-reports-2026-09-11).

## Filed reports (2026-09-11)

All three reports below are now live:

| Where | Link | Status as filed |
| --- | --- | --- |
| WebKitGTK (signature 1) | [bug 323949](https://bugs.webkit.org/show_bug.cgi?id=323949) | NEW |
| NVIDIA (signature 2) | [forum post](https://forums.developer.nvidia.com/t/egldestroycontext-segfault-in-libnvidia-eglcore-580-branch-during-ordinary-gl-context-teardown-likely-same-class-as-bug-5701801/382969) | posted, referencing existing bug 5701801 |

**Signature 1 already has a pre-existing report with the actual root cause: [bug 321683](https://bugs.webkit.org/show_bug.cgi?id=321683)**, filed 2026-08-13 by a different reporter (found independently — neither report referenced the other at filing time). Its title: *"[GTK] Release-build null-pointer crash in `AcceleratedBackingStore::update()` when `document.startViewTransition()` is the first trigger of accelerated compositing."* Its root-cause finding, which this doc's investigation never reached: three call sites in `WebKitWebViewBase.cpp` guard against a null accelerated backing store with `ASSERT()` rather than a real runtime check — and `ASSERT()` compiles to nothing in release builds (Ubuntu's WebKitGTK package included), so the null pointer sails through to `AcceleratedBackingStore::update()` and crashes there, exactly matching [this doc's resolved signature-1 backtrace](#signature-1-resolved-null-pointer-not-a-driver-frame). Proposed fix in 321683: replace the `ASSERT()`s with `if (!webkitWebViewBase->priv->acceleratedBackingStore) return;`. Workaround given there: disable the View Transitions feature via `webkit_settings_set_feature_enabled()`.

This closes the last open question from [Symbolized backtraces](#symbolized-backtraces-2026-09-11) about *why* `EnterAcceleratedCompositingMode` reaches a null backing store — **it's very likely a YouTube watch page's own frontend calls `document.startViewTransition()`** (common for SPA-style in-page navigation, e.g. switching between the video and its metadata panel) **while the embed player, which is a much simpler single-purpose page, does not** — which would explain the doc's original, still-unexplained observation that the embed player playing the identical video never crashes. Not independently confirmed by inspecting YouTube's own JS, but it fits every piece of evidence gathered here.

323949 is very likely a duplicate of 321683 and will probably be marked as such by a triager — that's fine; it still adds two things 321683 didn't have: independent confirmation via a completely different trigger (video playback, not deliberate View Transitions testing) demonstrating real-world prevalence beyond the API surface itself, and the discovery that this crash cascades into the separate NVIDIA `eglDestroyContext()` crash (via the WebProcess's self-termination-on-severed-IPC path documented above). Worth a comment on one of the two bugs cross-referencing the other, and noting to a triager that **fixing 321683 likely also eliminates the NVIDIA-side crash in practice** (the WebProcess would never reach the teardown path that trips the driver bug) even though `eglDestroyContext()` itself remains an independently real NVIDIA robustness issue.
...
- Cloned WebKit at the `webkitgtk-2.52.6` tag (matching the symbol versions used throughout this doc) into a scratch directory outside this repo (`~/Development/webkitgtk-321683-build/`, kept separate deliberately — a full WebKit checkout + build tree has nothing to do with this project).
- Applied the patch cleanly (`git apply --check` passed against the pristine tag).
- Built via Igalia's [`webkit-container-sdk`](https://github.com/Igalia/webkit-container-sdk) (Podman-based; upstream's currently-recommended way to build the GTK port) — one CMake dependency gap (`librice`, needed for `USE_LIBRICE`/WebRTC, not present in this container image) worked around with `--cmakeargs="-DUSE_LIBRICE=OFF"`, unrelated to what's being tested here. Clean build in 1h49m.
...
- ~~why is `AcceleratedBackingStore::update()` reachable with a null backing store~~ — **resolved 2026-09-11**: pre-existing [bug 321683](https://bugs.webkit.org/show_bug.cgi?id=321683) found it — `document.startViewTransition()` as the first trigger of accelerated compositing reaches three `ASSERT()`-guarded (i.e. release-build-stripped) null checks in `WebKitWebViewBase.cpp`. See [Filed reports](#filed-reports-2026-09-11) and [Patch validated locally](#patch-validated-locally-2026-09-11) — the proposed fix was built and confirmed to eliminate both signatures on this hardware.
