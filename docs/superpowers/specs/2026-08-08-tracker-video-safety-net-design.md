...
**Resolved 2026-08-09.** The AVIF/WebP image-codec theory above was also wrong. The crash is a bug in **NVIDIA's proprietary EGL driver**: `WebKitWebProcess` segfaults inside `libnvidia-eglcore`, reached via `libEGL_nvidia` from `libwebkit2gtk-4.1`, while compositing pages that play video. It is not a codec problem at all — forcing software VP9 decode still crashed, hardware NVDEC decoding works fine in the fixed configuration, and local HTML5 video (H.264/VP9, up to 4K), MSE, canvas/WebGL video upload and the YouTube embed player never crashed. Only selecting a different EGL vendor library avoided it, out of thirteen toggles tried. **This did not produce a usable fix:** the EGL-vendor switch was implemented as `egl_workaround.rs`, benchmarked at ~0.2 FPS against 62 FPS stock, and reverted — so the mitigations described in this document remain the real defence, not a historical footnote. See the Stage 1 known-issue entry in [ROADMAP.md](../../../ROADMAP.md) and [docs/WEBKITGTK-NVIDIA-EGL-CRASH.md](../../WEBKITGTK-NVIDIA-EGL-CRASH.md). This section is kept as-written for history — it records what was believed at the time, not what is true.
