...
**Resolved 2026-08-09.** The AVIF/WebP image-codec theory above was also wrong. The crash is a bug in **NVIDIA's proprietary EGL driver**: `WebKitWebProcess` segfaults inside `libnvidia-eglcore`, reached via `libEGL_nvidia` from `libwebkit2gtk-4.1`, while compositing pages that play video. It is not a codec problem at all — forcing software VP9 decode still crashed, hardware NVDEC decoding works fine in the fixed configuration, and local HTML5 video (H.264/VP9, up to 4K), MSE, canvas/WebGL video upload and the YouTube embed player never crashed. Only selecting a different EGL vendor library avoided it, out of eight toggles tried. Fixed at process start by `tauri-client/src-tauri/src/egl_workaround.rs`; see the Stage 1 known-issue entry in [ROADMAP.md](../../../ROADMAP.md) for the full bisection. This section is kept as-written for history — it records what was believed at the time, not what is true.
