...
**Fix:** `tauri-client/src-tauri/src/egl_workaround.rs` points WebKit at Mesa's EGL vendor library (`__EGL_VENDOR_LIBRARY_FILENAMES`) at process start, gated to Linux + NVIDIA 580.x, skipped if the user sets that variable themselves. Hardware video decoding is unaffected; only WebKit's GL compositing falls back to llvmpipe. Verified: the DDB homepage and YouTube watch pages both crash without it and run clean with it.

**Follow-up:** the two earlier mitigations are now belt-and-braces on NVIDIA 580 — `consts::DDB_URL` pointing at `/characters`, `safety_net.rs`'s video-stripping, and `homepage_redirect.rs` + `on_navigation` in `lib.rs`. They were built when the cause was unknown and are retained deliberately (they also cover any machine where the workaround doesn't apply), but the homepage redirect in particular is now a candidate for removal — decide that on its own merits rather than as part of this fix. The tracker-blocking half of `safety_net.rs` has independent value and should stay regardless. See [docs/superpowers/specs/2026-08-08-tracker-video-safety-net-design.md](docs/superpowers/specs/2026-08-08-tracker-video-safety-net-design.md) for the full history including both disproved theories.
...
**Retest this first (2026-08-09):** the Stage 1 homepage crash above now has a confirmed root cause — a bug in NVIDIA's proprietary EGL driver — and a fix in `egl_workaround.rs`. If the "same underlying media-rendering issue" theory holds, animated map backgrounds may simply render correctly now. This was not verified, because reproducing it needs a real campaign with an animated map behind a DDB login. Cheapest check before investigating anything else: open such a map with the fix active, then again with `__EGL_VENDOR_LIBRARY_FILENAMES=/usr/share/glvnd/egl_vendor.d/10_nvidia.json` set to force the old path back on, and compare.
