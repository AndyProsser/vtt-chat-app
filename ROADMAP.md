...
**Plan A implemented 2026-08-31 — compact/expanded UI, corner positioning, avatar placeholders, twelve commits.** `classifyPage`/`usePageMode` and the `FullPanel`/`MicPill` split (described above) are gone, replaced by `CompactPanel` (the one default view everywhere — mute icon, divider, avatar row, expand toggle) and `ExpandedPanel` (opt-in vertical view, same underlying participant data), switched per-instance by `expandStore` (UI-only, never persisted — every fresh mount starts collapsed, per the redesign spec's reasoning that critical state should never hide behind a preference nobody remembers setting). `corner` adds per-corner positioning, persisted to `localStorage` and read once per window load, changed via `OverlayCornerMenu`'s right-click picker (a Radix `ContextMenu` wrapping the whole panel). `Avatar` absorbs the deleted `SpeakingDot`'s leaf-isolated role (see `docs/architecture/STATE-AND-RESILIENCE.md`) plus a deterministic placeholder color (`avatarColor`, hashed from participant identity) — still a color swatch, not a portrait or display name, until Stage 3b's DDB extraction lands. `MuteIcon` replaces `MuteButton`: icon-only, with a Radix `Tooltip` carrying the state detail that used to be an always-visible label, plus a third visual state ("not connected") distinct from "muted." Overlay-everywhere is now the actual, intentional behavior — see [CLAUDE.md §8.1](CLAUDE.md)'s 2026-08-31 note — rather than the "scoped to Maps + debug toggle" deliverable described above, which predates the 2026-08-14 redesign. Bundle grew again: `overlay.js` 803.43KB → 920.77KB (gzip 155.23KB → 192.31KB), Radix's `ContextMenu` and `Tooltip` pulling in `@radix-ui/react-menu`, `react-popper`, `floating-ui`, and friends — the same accepted-cost tradeoff as the original Radix wiring above, not a regression.

**Final review found the corner menu and both tooltips effectively unusable, fixed the same day.** Radix's `Tooltip`/`ContextMenu.Content` portal to `document.body` by default, which sits outside the Shadow DOM everything else here is scoped to — so both rendered fully unstyled and behind DDB's own content, past the host's max z-index, until every call site was given an explicit `container` prop pointing at a dedicated element inside the shadow tree (`ShadowPortalContainerContext`, `lib/shadowRootContext.tsx`, threaded through from `main.tsx`). Separately, `.vtt-overlay`'s `pointer-events: none` only opted back in on `button`, so `OverlayCornerMenu`'s right-click trigger — attached to the whole panel via `ContextMenu.Trigger`'s `asChild` — was unreachable anywhere except the two icon buttons themselves; the panel background, divider, and avatar row all passed the click straight through to DDB underneath. Fixed by moving the pointer-events opt-in to `.vtt-compact-panel`/`.vtt-expanded-panel`. Neither defect is the kind of thing lint/typecheck/build catches, and both survived all twelve implementation commits' own task reviews — caught only by a whole-branch review reasoning through the Shadow DOM + portal + pointer-events interaction directly. See git history 2026-08-31 for the fix commits.
